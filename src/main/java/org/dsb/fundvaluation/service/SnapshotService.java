...
public class SnapshotService {
    ...
    public SnapshotResponse getSnapshot() {
        ...
        try {
            ...
            copy.setGeneratedAt(currentSnapshot.getGeneratedAt());
            copy.setFunds(currentSnapshot.getFunds() != null
                    ? List.copyOf(currentSnapshot.getFunds()) : List.of());
            copy.setDetails(currentSnapshot.getDetails() != null
                    ? Collections.unmodifiableMap(new LinkedHashMap<>(currentSnapshot.getDetails()))
                    : Map.of());
            return copy;
        } finally {
    ...
    private void refreshQuoteData() {
        ...
        try {
            for (FundData fund : fundDataService.getAllFunds()) {
                ...
                if (hadOldNav) {
                    BigDecimal baseNav = BigDecimal.valueOf(detail.getBaseNav());
    ...
    private List<HoldingDetail> buildEnrichedHoldings(FundData fund, Map<String, Quote> quotes) {
        ...
        for (HoldingData holding : fund.getHoldings()) {
            ...
            if (quote == null) {
                ...
                BigDecimal contribution = ratio.multiply(stockReturn);
                ...
                row.setWeightContribution(contribution.doubleValue());
                row.setStatus("ok");
            }
            enriched.add(row);
        }
